...
void entry(unsigned long magic, unsigned long addr) {
    ...
    /* Construct a IDT entry in the GDT */
    {
        int i;
        for(i = 0; i <= 0x1F; i++){
					//the number 16 is due to our alignment in our IDT_JMP_TBL
					idt[i].offset_15_00 = (((uint32_t)&IDT_JMP_TBL) + i * 16) & 0x0000FFFF;
					...
					idt[i].dpl       = 0;
					idt[i].present   = 1;
					idt[i].offset_31_16 = ((((uint32_t)&IDT_JMP_TBL) + i * 16) & 0xFFFF0000) >> 16;
        }
        ...
        idt[0x80].dpl       = 3;
		idt[0x20].offset_15_00 = ((uint32_t)&switch_process) & 0x0000FFFF;
		idt[0x20].offset_31_16 = (((uint32_t)&switch_process) & 0xFFFF0000) >> 16;
    ...
    asm volatile ("                 \n\
            ...
            "
            :
            : "a"(0), "D"(0x7f0000), "c"(65536)
            : "edx", "memory", "cc"
    );
	asm volatile ("mov $0x002B, %cx		\n\
					mov %cx, %ds"
	);
	execute((uint8_t*)"shell", 0);
	execute((uint8_t*)"shell", 1);
	execute((uint8_t*)"shell", 2);
